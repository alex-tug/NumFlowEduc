...
def calc_transp_CN(pd, m, to_step):    # pd ... project data
    ...
    th = 0.5   # theta, Crank-Nicolson is stable for theta >= 0.5
    ...
    u_right = pd.u_00.copy()
    try:
        for n in range(1, to_step):
            #u_right = np.dot(B_band, u_0)
            u_right[1:-1] =  (+0.5*th_inv*pd.CFL  +    th_inv*pd.NE      ) * u_0[0:-2]\
                            +(                    -2.0*th_inv*pd.NE  +1.0) * u_0[1:-1]\
                            +(-0.5*th_inv*pd.CFL  +    th_inv*pd.NE      ) * u_0[2:]
            u_right[0] = pd.bc_upstream(n*pd.dt)
            u_right[-1] = pd.bc_downstream(n*pd.dt)

            #u_1 = np.dot(A_inv, np.dot(B, u_0))
            #u_1 = np.dot(A_inv, u_right)
            u_1 = la.solve_banded((1, 1), A_data, u_right)
